...
export default function useResumableSSE(
  ...
  const subscribeToStream = useCallback(
    (currentStreamId: string, currentSubmission: TSubmission, isResume = false) => {
      ...
      sse.addEventListener('error', async (e: MessageEvent) => {
        ...
        // 404 means job doesn't exist (completed/deleted) - don't retry, show error to user
        if (responseCode === 404) {
          ...
          errorHandler({
            data: {
              text: 'Stream não encontrado no servidor. A geração pode ter falhado ao iniciar ou expirado. Tente enviar a mensagem novamente.',
  ...
  const startGeneration = useCallback(
    async (currentSubmission: TSubmission): Promise<string | null> => {
      ...
      const START_GENERATION_TIMEOUT_MS = 90_000; // 90s - backend must return streamId quickly
        ...
        try {
          ...
          const timeoutPromise = new Promise<never>((_, reject) => {
            timeoutId = setTimeout(
              () => reject(new Error('START_GENERATION_TIMEOUT')),
              START_GENERATION_TIMEOUT_MS,
            );
          });
          const data = (await Promise.race([
            request.post(url, payload).finally(() => clearTimeout(timeoutId)),
            timeoutPromise,
          ])) as { streamId?: string };
          const streamIdFromServer = data?.streamId;
          if (!streamIdFromServer) {
            console.error('[ResumableSSE] Backend response missing streamId:', data);
            lastError = new Error('Resposta do servidor sem streamId');
            break;
          }
          console.log('[ResumableSSE] Generation started:', { streamId: streamIdFromServer });
          return streamIdFromServer;
        } catch (error) {
          lastError = error;
      ...
      const errMessage =
        lastError instanceof Error && lastError.message === 'START_GENERATION_TIMEOUT'
          ? 'O servidor demorou para responder. Verifique os logs do backend e tente novamente.'
          : null;
      ...
      if (errMessage) {
        errorHandler({
          data: { text: errMessage } as unknown as Parameters<typeof errorHandler>[0]['data'],
          submission: currentSubmission as EventSubmission,
        });
      } else if (errorData) {
        errorHandler({
