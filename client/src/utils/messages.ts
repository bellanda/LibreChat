...
/** Appends index suffix to agent ID (mirrors librechat-data-provider when not exported). */
function appendAgentIdSuffix(agentId: string, index: number): string {
  return `${agentId}__${index}`;
}

/** Encodes ephemeral agent ID from endpoint/model/sender (mirrors librechat-data-provider when not exported). */
function encodeEphemeralAgentId({
  endpoint,
  model,
  sender,
  index,
  ...
  const payload = [endpoint, model, sender, index ?? 0].join('\0');
  return `${Constants.EPHEMERAL_AGENT_ID}_${btoa(encodeURIComponent(payload)).replace(/=/g, '')}`;
}
...
export const createDualMessageContent = (
  ...
  if (primaryConvo.agent_id && !isEphemeralAgent(primaryConvo.agent_id)) {
  ...
  if (addedConvo.agent_id && !isEphemeralAgent(addedConvo.agent_id)) {
