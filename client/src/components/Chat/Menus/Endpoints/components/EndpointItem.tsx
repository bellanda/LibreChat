  ...
  if (endpoint.hasModels) {
    ...
    return (
      <MenuItem
        ...
        key={`endpoint-${endpoint.value}-item`}
        onClick={() => handleSelectEndpoint(endpoint)}
        className="group flex w-full cursor-pointer items-center justify-between rounded-lg p-2 text-sm"
      >
        {renderIconLabel()}
        <div className="flex shrink-0 items-center gap-2">
          ...
          {isAssistantsNotLoaded && (
            <span className="flex items-center" aria-label={localize('com_ui_loading')}>
