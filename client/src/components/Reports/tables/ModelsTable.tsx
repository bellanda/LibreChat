...
export default function ModelsTable({ data, isLoading }: ModelsTableProps) {
  ...
  const costFilterFn = React.useCallback(
    (row: any) => {
      ...
      // Se não há custo definido, incluir no filtro apenas se min for 0 ou vazio
      if (cost === undefined || cost === null) {
        return !costFilter.min && !costFilter.max;
      }
    ...
    <div className="rounded-xl border border-gray-700/50 bg-[#1c1c1c] p-5">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-white">
          {REPORT_LABELS.TABLES.MODELS_MESSAGES_COST}
        </h3>
        <p className="text-sm text-gray-400">
          {totalFiltered > 0
            ? `${startItem}-${endItem} de ${totalFiltered} modelos` +
              (totalFiltered !== totalData ? ` (filtrado de ${totalData})` : '')
            : `0 de ${totalData} modelos`}
        </p>
      </div>
      ...
      <div className="mb-4 space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border border-gray-600/50 bg-gray-800/50 py-2 pl-10 pr-4 text-white placeholder:text-gray-400 focus:border-blue-500/50 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
          />
        </div>
        ...
        <div className="flex items-center justify-between px-2 py-4">
          ...
          <div className="flex items-center space-x-2">
            <Button
              ...
              className="border-gray-600/50 bg-gray-800/50 text-gray-300 hover:bg-gray-700/50 hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
            >
