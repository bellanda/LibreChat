...
import { ModelData, REPORT_LABELS } from '~/store/reports';
  ...
  export default function CostCentersTable({ data, isLoading }: CostCentersTableProps) {
      ...
      () => [
        {
          ...
          cell: ({ row }) => (
            <div className="text-white font-mono font-bold">{row.getValue('code')}</div>
          ),
        },
        {
          accessorKey: 'name',
      ...
      <div className="bg-[#1c1c1c] p-5 rounded-xl border border-gray-700/50">
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-white">
            {REPORT_LABELS.TABLES.COST_CENTERS_MESSAGES_COST}
          </h3>
          <p className="text-sm text-gray-400">
            {totalFiltered > 0 
              ? `${startItem}-${endItem} de ${totalFiltered} centros de custo` + (totalFiltered !== totalData ? ` (filtrado de ${totalData})` : '')
              : `0 de ${totalData} centros de custo`
            }
          </p>
        </div>
