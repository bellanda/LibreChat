...
class ExtendedPgVector(PGVector):
    ...
    @staticmethod
    def _sanitize_parameters_for_logging(parameters: Union[Dict, List, tuple, Any]) -> Any:
        ...
        if isinstance(parameters, dict):
            ...
            for key, value in parameters.items():
                ...
                if "embedding" in str(key).lower() or (
                    ...
                    sanitized[key] = ExtendedPgVector._sanitize_parameters_for_logging(value)
                else:
                    sanitized[key] = value
            return sanitized
            ...
            for item in parameters:
                if (
                    ...
                    sanitized.append(ExtendedPgVector._sanitize_parameters_for_logging(item))
                else:
    ...
    def setup_query_logging(self):
        ...
        def receive_before_cursor_execute(conn, cursor, statement, parameters, context, executemany):
            if "langchain_pg_embedding" in statement:
                ...
                sanitized_params = ExtendedPgVector._sanitize_parameters_for_logging(parameters)
    ...
    def get_documents_by_ids(self, ids: list[str]) -> list[Document]:
        with Session(self._bind) as session:
            results = session.query(self.EmbeddingStore).filter(self.EmbeddingStore.custom_id.in_(ids)).all()
            return [
