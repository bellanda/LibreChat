...
export class ConnectionsRepository {
  ...
  async has(serverName: string): Promise<boolean> {
    const config = await mcpServersRegistry.getServerConfig(serverName, this.ownerId);
  ...
  async get(serverName: string): Promise<MCPConnection | null> {
    const serverConfig = await mcpServersRegistry.getServerConfig(serverName, this.ownerId);
  ...
  async getMany(serverNames: string[]): Promise<Map<string, MCPConnection>> {
    ...
    return new Map(
      (connections as [string, MCPConnection | null][]).filter(
        (v): v is [string, MCPConnection] => v[1] != null,
      ),
    );
  }
  ...
  async getAll(): Promise<Map<string, MCPConnection>> {
    ...
    const allConfigs = await mcpServersRegistry.getAllServerConfigs(this.ownerId);
